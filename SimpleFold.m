...
AnalyInputOpt = struct(...
    ...
    'AdaptiveLoad', @LoadFunSimpleFold, ...
    'DispStep', 200);
...
RotateAngle = zeros(1, length(Uhis(1, :)));
hold off
for i = 1:length(Uhis(1, :))
    RotateAngle(i) = atan((Node(4,1)+Uhis(10,i))/(Node(4,3)+Uhis(12, i)));
end
plot(RotateAngle, Fhis, 'LineWidth',4);
